...
function buildPermissionPayload(perm, cwd) {
...
export const WarpPlugin = async ({ client, directory }) => {
    ...
    if (!process.env.WARP_CLI_AGENT_PROTOCOL_VERSION) {
        client.app
            ...
            body: {
                ...
                level: "warn",
                message: "⚠️ Detected unsupported Warp version. Please update Warp to use this plugin.",
            },
        })
            .catch((err) => {
    ...
    async function isSubagentSession(sessionId) {
        if (!sessionId)
            return false;
        if (subagentCache.has(sessionId))
            return subagentCache.get(sessionId);
        try {
        ...
        event: async ({ event }) => {
            ...
            switch (event.type) {
                ...
                case "permission.updated": {
                    await maybeWarpNotify(event.properties.sessionID, buildPermissionPayload(event.properties, cwd));
                    return;
                }
